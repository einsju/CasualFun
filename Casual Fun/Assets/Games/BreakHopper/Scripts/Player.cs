...
namespace CasualFun.Games.BreakHopper
{
	public class Player : MonoBehaviour
	{
		...
		void Update()
		{
			...
			if (!animPlaying)
				mesh.localScale = startScale + Vector3.up * Mathf.Abs(rb.velocity.y) * stretchAmount +
				                  Vector3.right * -Mathf.Abs(rb.velocity.y) * stretchAmount / 3f;
		...
		bool PaintClose()
		{
			...
			{
				if (Vector3.Distance(transform.position, paint[i].transform.position) < paintDistance &&
				    paint[i].GetComponent<Paint>().GetColor() == color)
				{
					count++;
